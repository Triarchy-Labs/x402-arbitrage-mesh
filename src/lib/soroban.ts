...
const USDC_ISSUER = process.env.STELLAR_USDC_ISSUER || "GBBD47IF6LWK7P7MDEVSCWTTCJM4NTC3L3P6HZ2ZNGQ5ZCDNDGNQ5V5D";
 ...
 */
export async function validateSorobanPayment(txHash: string, requiredAmount: number, expectedMemo: string): Promise<PaymentValidationResult> {
    try {
        // Fetch transaction details
        const txResp = await fetch(`${HORIZON_TESTNET_URL}/transactions/${txHash}`);
        if (!txResp.ok) {
            return { valid: false, error: "Transaction not found on Stellar Testnet." };
        }
        ...
        if (txData.memo_type !== "text" || txData.memo !== expectedMemo) {
            return { valid: false, error: `Invalid Memo. Expected '${expectedMemo}', got '${txData.memo}'.` };
        }

        // Fetch operations to verify the payment amount and destination
        const opsResp = await fetch(`${HORIZON_TESTNET_URL}/transactions/${txHash}/operations`);
        if (!opsResp.ok) {
        ...
        if (!paymentFound) {
            return { valid: false, error: "No valid payment to the Enterprise Platform Wallet found in this transaction." };
        }

        if (totalPaid < requiredAmount) {
            return { valid: false, error: `Insufficient funds. Paid ${totalPaid}, required ${requiredAmount}.` };
        }
        ...
        return { valid: false, error: "Internal validation error connecting to Horizon RPC." };
    }
}
