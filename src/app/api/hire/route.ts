...
export async function POST(req: Request) {
	try {
		...
		const authHeader =
			req.headers.get("authorization") || req.headers.get("x-l402-txhash");
		...
		if (!txHash) {
			if (l402Mode === "subscription") {
				return NextResponse.json(
					{
						error:
							"Subscription Payment Required. Please provide valid recurring txhash via L402.",
					},
					{
						status: 402,
						headers: {
							"WWW-Authenticate":
								'L402 invoice="soroban_subscription_required"',
						},
					},
				);
			}
			return NextResponse.json(
				{
					error:
						"Payment Required. Please provide x-l402-txhash header with a valid Stellar Testnet Transaction Hash.",
				},
				{
					status: 402,
					headers: {
						"WWW-Authenticate": 'L402 invoice="soroban_payment_required"',
					},
				},
			);
		}
		...
		if (!description || bounty_usdc === undefined) {
			return NextResponse.json(
				{ error: "Missing required fields: description, bounty_usdc" },
				{ status: 400 },
			);
		}
		...
		const isBypass =
			...
			process.env.DEV_BYPASS_HASH &&
			txHash === process.env.DEV_BYPASS_HASH;
		if (!isBypass) {
			const expectedMemo = payload_id || client_id || task_id || "demo";
			const validation = await validateSorobanPayment(
				txHash,
				price,
				expectedMemo,
			);
			if (!validation.valid) {
				return NextResponse.json(
					{ error: `x402 Payment Validation Failed: ${validation.error}` },
					{ status: 402 },
				);
			}
		}
		...
		if (!sandboxResult.safe) {
			console.warn(
				`[OPSEC FIREWALL] Blocked malicious payload from ${client_id}. USDC ${price} refunded.`,
			);
			return NextResponse.json(
				{
					status: "rejected",
		...
		if (price > 0 && price < ENTERPRISE_THRESHOLD && !isQueueFull) {
			try {
				...
				console.warn(
					"Local micro-node unavailable, escalating to P2P network.",
				);
			}
		}
